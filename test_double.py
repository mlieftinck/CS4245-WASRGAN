...
def load_dataset(config: Any, device: torch.device) -> CUDAPrefetcher:
    test_datasets_1 = PairedImageDataset(config["TEST"]["DATASET"]["PAIRED_TEST_GT_IMAGES_DIR_1"],
                                       config["TEST"]["DATASET"]["PAIRED_TEST_LR_IMAGES_DIR_1"])
    test_datasets_2 = PairedImageDataset(config["TEST"]["DATASET"]["PAIRED_TEST_GT_IMAGES_DIR_2"],
                                         config["TEST"]["DATASET"]["PAIRED_TEST_LR_IMAGES_DIR_2"])
    test_dataloader_1 = DataLoader(test_datasets_1,
    ...
    test_dataloader_2 = DataLoader(test_datasets_2,
                                   batch_size=config["TEST"]["HYP"]["IMGS_PER_BATCH"],
                                   shuffle=config["TEST"]["HYP"]["SHUFFLE"],
                                   num_workers=config["TEST"]["HYP"]["NUM_WORKERS"],
                                   pin_memory=config["TEST"]["HYP"]["PIN_MEMORY"],
                                   drop_last=False,
                                   persistent_workers=config["TEST"]["HYP"]["PERSISTENT_WORKERS"])
    test_test_data_prefetcher_1 = CPUPrefetcher(test_dataloader_1)
    test_test_data_prefetcher_2 = CPUPrefetcher(test_dataloader_2)
    return test_test_data_prefetcher_1, test_test_data_prefetcher_2
...
def test(name_test: str,
    ...
    if config["TEST"]["SAVE_IMAGE_DIR_1"] and name_test == "whatsapp":
        save_image = True
        save_image_dir = os.path.join(config["TEST"]["SAVE_IMAGE_DIR_1"],
                                      config["EXP_NAME"] + "_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
        make_directory(save_image_dir)
    elif config["TEST"]["SAVE_IMAGE_DIR_1"] and name_test == "bicubic":
        save_image = True
        save_image_dir = os.path.join(config["TEST"]["SAVE_IMAGE_DIR_2"],
                                      config["EXP_NAME"] + "_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
        make_directory(save_image_dir)
...
def main() -> None:
    ...
    test_data_prefetcher_app, test_data_prefetcher_bicubic = load_dataset(config, device)
    ...
    test("whatsapp", g_model,
         test_data_prefetcher_app,
         psnr_model,
         ssim_model,
         device,
         config)

    test("bicubic", g_model,
         test_data_prefetcher_app,
         psnr_model,
         ssim_model,
         device,
